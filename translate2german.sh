...
sudo rpl --encoding UTF-8 'Offline</span>' 'FTL ist aus</span>' /var/www/html/admin/scripts/pi-hole/php/header.php
sudo rpl --encoding UTF-8 '</i> Offline' '</i> FTL ist aus' /var/www/html/admin/scripts/pi-hole/js/footer.js
...
sudo rpl --encoding UTF-8 '<span>Custom time</span>' '<span>Benutzerdefinierte Zeit</span>' /var/www/html/admin/scripts/pi-hole/php/header.php
...
sudo rpl --encoding UTF-8 '</i> Query Lists' '</i> Domain in einer Liste finden' /var/www/html/admin/scripts/pi-hole/php/header.php
sudo rpl --encoding UTF-8 '</i> Audit log' '</i> Prüfprotokoll' /var/www/html/admin/scripts/pi-hole/php/header.php
sudo rpl --encoding UTF-8 '</i> Tail pihole.log' '</i> pihole.log anzeigen' /var/www/html/admin/scripts/pi-hole/php/header.php
sudo rpl --encoding UTF-8 '</i> Tail pihole-FTL.log' '</i> pihole-FTL.log anzeigen' /var/www/html/admin/scripts/pi-hole/php/header.php
...
# evtl. für Speedtest sudo rpl --encoding UTF-8 'Speedtest results over last' 'Geschwindigkeitsergebnisse der letzten' /var/www/html/admin/index.php
...
sudo rpl --encoding UTF-8 '</span>, sent to ' '</span>, gesendet an ' /var/www/html/admin/scripts/pi-hole/js/queries.js
sudo rpl --encoding UTF-8 '</span>, answered by ' '</span>, beantwortet von ' /var/www/html/admin/scripts/pi-hole/js/queries.js
sudo rpl --encoding UTF-8 'Blocked \(gravity\)' 'Geblockt (Gravity)' /var/www/html/admin/scripts/pi-hole/js/db_queries.js
sudo rpl --encoding UTF-8 'forwarded to ' 'weitergeleitet an: ' /var/www/html/admin/scripts/pi-hole/js/db_queries.js
...
sudo rpl --encoding UTF-8 'Blocked \(gravity, CNAME\)' 'Geblockt (gravity, CNAME)' /var/www/html/admin/scripts/pi-hole/js/db_queries.js
...
sudo rpl --encoding UTF-8 'Blocked \(gravity, CNAME\)' 'Geblockt (Gravity, CNAME)' /var/www/html/admin/scripts/pi-hole/js/queries.js
...
# folgt noch
# sudo rpl --encoding UTF-8 'Adding' 'Hinzufügen' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'to the' 'zum' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'Success!' 'Erfolgreich!' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'Failure! Something went wrong.' 'Fehler! Etwas ist schief gelaufen.' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'Recent Speedtests' 'Aktueller Bandbreitentest' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'Test Time' 'Testzeit' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'End Time' 'Endzeit' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'Your IP' 'Ihre IP-Adresse' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'Distance' 'Entfernung' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 'Results' 'Ergebnisse' /var/www/html/admin/speedtest.php
# sudo rpl --encoding UTF-8 '> View Result</a' '> Ansehen</a' /var/www/html/admin/scripts/pi-hole/js/speedresults.js
...
sudo rpl --encoding UTF-8 '"Added " . htmlentities\(utf8_encode\(\$domain\)\);' '"Hinzugefügt: " . htmlentities(utf8_encode($domain));' /var/www/html/admin/scripts/pi-hole/php/groups.php
...
sudo rpl --encoding UTF-8 '<li>Multiple adlists can be added by separating each <i>unique</i> URL with a space</li>' '<li>Mehrere Adlists können hinzugefügt werden, indem jede <i>eindeutige</i> URL durch ein Leerzeichen getrennt wird.</li>' /var/www/html/admin/groups-adlists.php
...
sudo rpl --encoding UTF-8 'List download was successful \(<span class="list-status-1">OK</span>\)' 'Herunterladen der Liste war erfolgreich. (<span class="list-status-1">OK</span>)' /var/www/html/admin/scripts/pi-hole/js/groups-adlists.js
...
sudo rpl --encoding UTF-8 '<th>Action</th>' '<th>Aktion</th>' /var/www/html/admin/messages.php
...
sudo rpl --encoding UTF-8 '"Deleting message with ID "' '"Gelöschte Mitteilung mit der  "' /var/www/html/admin/scripts/pi-hole/js/messages.js
sudo rpl --encoding UTF-8 '"Successfully deleted message # "' '"Mitteilung erfolgreich gelöscht # "' /var/www/html/admin/scripts/pi-hole/js/messages.js
sudo rpl --encoding UTF-8 '"Error while deleting message with ID "' '"Fehler beim Löschen der Mitteilung mit der ID "' /var/www/html/admin/scripts/pi-hole/js/messages.js
...
sudo rpl --encoding UTF-8 'Using libz compression\n' 'Verwenden der libz-Komprimierung\n' /opt/pihole/gravity.sh
...
sudo rpl --encoding UTF-8 'Domain to look for \(example.com or sub.example.com\)' 'zu suchende Domain (beispiel.com oder mobile.beispiel.com)' /var/www/html/admin/queryads.php
...
sudo rpl --encoding UTF-8 'Output the last lines of the pihole-FTL.log file \(live\)' 'Ausgabe der letzten Zeilen der Datei pihole-FTL.log (live)' /var/www/html/admin/taillog-FTL.php
...
sudo rpl --encoding UTF-8 'Upload debug log and provide token once finished' 'Laden Sie das Debug-Protokoll hoch und stellen Sie das Token bereit, sobald es fertig ist.' /var/www/html/admin/debug.php
...
# Einstellungen
sudo rpl --encoding UTF-8 'Debug</h4>' 'Fehlersuche</h4>' /var/www/html/admin/settings.php
...
sudo rpl --encoding UTF-8 "<p>If not configured as your DHCP server, Pi-hole typically won't be able to" '<p>Wenn Pi-hole nicht als DHCP-Server konfiguriert ist, kann' /var/www/html/admin/settings.php
...
sudo rpl --encoding UTF-8 'Styling \(auto saved, per-browser\)' 'Aussehen (wird automatisch gespeichert)' /var/www/html/admin/settings.php
...
#  Geschwindigkeitstest
#  folgt noch
#sudo rpl --encoding UTF-8 '>Speedtest<' '>Geschwindigkeitstest<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Speedtest settings' 'Einstellungen für Geschwindigkeitstest' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Speedtest Schedule' 'Zeitplan für den Geschwindigkeitstest' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 '>Disabled<' '>Ausgeschaltet<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 '>Every 1 Hour<' '>Jede Stunde<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 '>Every 2 Hours<' '>Alle 2 Stunden<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 '>Every 4 Hours<' '>Alle 4 Stunden<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 '>Every 6 Hours<' '>Alle 6 Stunden<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 '>Every 12 Hours<' '>Alle 12 Stunden<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 '>Every 24 Hours<' '>Alle 24 Stunden<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Speedtest Display Range' 'Geschwindigkeitstest Anzeigebereich' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Day<' 'Tag<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Days<' 'Tage<' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Custom Speedtest server' 'Benutzerdefinierter Server für den Geschwindigkeitstest' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Expert only!!!</span>. Get list of supported servers <a href="https://www.speedtest.net/speedtest-servers.php" target="_blank"> here</a></p>' 'Nur für Experten!!!</span>. Eine Liste der unterstützten Server finden Sie <a href="https://www.speedtest.net/speedtest-servers.php" target="_blank"> hier</a>.</p>' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'Flush Speedtest history' 'Löschen der Geschwindigkeitstest-Hitorie.' /var/www/html/admin/settings.php
#sudo rpl --encoding UTF-8 'The Speedtest settings have been updated' 'Die Speedtest Einstellungen wurden aktualisiert.' /var/www/html/admin/scripts/pi-hole/php/savesettings.php
...
sudo rpl --encoding UTF-8 'Updating the custom DNS entries...' 'Aktualisieren der benutzerdefinierten DNS-Einträge ...' /var/www/html/admin/dns_records.php
sudo rpl --encoding UTF-8 'Success! The list will refresh.' 'Erfolgreich! Die Liste wurde aktualisiert.' /var/www/html/admin/dns_records.php
sudo rpl --encoding UTF-8 'Failure! Something went wrong, see output below' 'Fehler! Irgendwas hat nicht funktioniert, siehe weiter unten' /var/www/html/admin/dns_records.php
sudo rpl --encoding UTF-8 'At least one domain was already present, see output below:<br/>' 'Mindestens eine Domäne war bereits vorhanden, siehe Ausgabe unten:<br/>' /var/www/html/admin/dns_records.php
...
sudo rpl --encoding UTF-8 'Error while adding this custom DNS entry' 'Fehler beim Hinzufügen dieses benutzerdefinierten DNS-Eintrags!' /var/www/html/admin/scripts/pi-hole/js/customdns.js
sudo rpl --encoding UTF-8 'Error while deleting this custom DNS entry' 'Fehler beim Löschen dieses benutzerdefinierten DNS-Eintrags!' /var/www/html/admin/scripts/pi-hole/js/customdns.js
...
sudo rpl --encoding UTF-8 'Updating CNAME records...' 'Aktualisiere die CNAME Einträge ...' /var/www/html/admin/cname_records.php
sudo rpl --encoding UTF-8 'Success! The list will refresh.' 'Erfolgreich! Die Liste wurde aktualisiert.' /var/www/html/admin/cname_records.php
sudo rpl --encoding UTF-8 'Failure! Something went wrong, see output below:' 'Fehler! Etwas ist schief gelaufen, siehe Ausgabe unten:' /var/www/html/admin/cname_records.php
sudo rpl --encoding UTF-8 'At least one domain was already present, see output below:' 'Mindestens eine Domäne war bereits vorhanden, siehe Ausgabe unten:' /var/www/html/admin/cname_records.php
